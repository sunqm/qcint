...
(defun cell-converter-avx (cell fout &optional (with-grids nil))
  (let ((fac (realpart (phase-of cell)))
        (const@3 (ternary-subscript (consts-of cell)))
        (op@3    (ternary-subscript (ops-of cell))))
    (cond ((equal fac 1)
...
;;; _cart
      (format fout "CACHE_SIZE_T ~a_cart(double *out, int *dims, int *shls,
...
;;; _sph
      (format fout "CACHE_SIZE_T ~a_sph(double *out, int *dims, int *shls,
int *atm, int natm, int *bas, int nbas, double *env, CINTOpt *opt, double *cache) {~%" intname)
      (format fout envs-common)
      (when (member 'g raw-infix)
        (when (or (member 'g bra-i) (member 'g ket-j))
...
;;; _spinor
      (format fout "CACHE_SIZE_T ~a_spinor(double complex *out, int *dims, int *shls,
int *atm, int natm, int *bas, int nbas, double *env, CINTOpt *opt, double *cache) {~%" intname)
      (format fout envs-common)
      (when (member 'g raw-infix)
        (when (or (member 'g bra-i) (member 'g ket-j))
...
(defun dump-s-loop-grids (fout tot-bits)
  (loop
    for i upto (1- (expt 3 tot-bits)) do
    (let* ((ybin (dec-to-ybin i))
           (zbin (dec-to-zbin i))
           (xbin (- (ash 1 tot-bits) 1 ybin zbin)))
      (format fout "rs[~a] += MM_LOAD(g~a+ix+ig+i*GRID_BLKSIZE) * MM_LOAD(g~a+iy+ig+i*GRID_BLKSIZE) * MM_LOAD(g~a+iz+ig+i*GRID_BLKSIZE);~%"
              i xbin ybin zbin i)))) ; end do i = 1, envs->nrys_roots
...
c2s_dset0(out+nout*i, dims, counts); }
return 0; }~%")))
      (format fout "return CINT3c2e_drv(out, dims, &envs, opt, cache, &c2s_sph_3c2e1, 0);~%} // ~a_sph~%" intname)
;;; _spinor
      (format fout "CACHE_SIZE_T ~a_spinor(double complex *out, int *dims, int *shls,
int *atm, int natm, int *bas, int nbas, double *env, CINTOpt *opt, double *cache) {~%" intname)
      (format fout envs-common)
...
c2s_dset0(out+nout*i, dims, counts); }
return 0; }~%")))
      (format fout "return CINT3c1e_drv(out, dims, &envs, opt, cache, &c2s_cart_3c1e);~%} // ~a_cart~%" intname)
;;; _sph
      (format fout "CACHE_SIZE_T ~a_sph(double *out, int *dims, int *shls,
int *atm, int natm, int *bas, int nbas, double *env, CINTOpt *opt, double *cache) {~%" intname)
      (format fout envs-common)
...
c2s_dset0(out+nout*i, dims, counts); }
return 0; }~%")))
      (format fout "return CINT3c1e_drv(out, dims, &envs, opt, cache, &c2s_sph_3c1e);~%} // ~a_sph~%" intname)
;;; _spinor
      (format fout "CACHE_SIZE_T ~a_spinor(double complex *out, int *dims, int *shls,
int *atm, int natm, int *bas, int nbas, double *env, CINTOpt *opt, double *cache) {~%" intname)
      (format fout envs-common)
...
(defun gen-code-gout1e-grids (fout intname raw-infix flat-script)
  (destructuring-bind (op bra-i ket-j bra-k ket-l)
    ...
    (let* ((i-rev (effect-keys bra-i)) ;<i| already in reverse order
           ...
           (goutinc (length flat-script)))
      (format fout "void CINTgout1e_~a" intname)
      (format fout "(double *RESTRICT gout, double *RESTRICT g,
...
int nf = envs->nf;
int nrys_roots = envs->nrys_roots;~%" goutinc)
      (format fout "int ix, iy, iz, n, i, ig;
double *RESTRICT g0 = g;~%")
      (loop
        for i in (range (num-g-intermediates tot-bits op i-len j-len)) do
        (format fout "double *RESTRICT g~a = g~a  + envs->g_size * 3;~%" (1+ i) i))
      (dump-declare-dri-for-rc fout bra-i "i")
      (dump-declare-dri-for-rc fout (append op ket-j) "j")
      (dump-declare-giao-ij fout bra-i (append op ket-j))
      (format fout "__MD rs[~a];~%" comp)
...
for (i = 0; i < ~a; i++) { rs[i] = MM_SET1(0.); }
for (i = 0; i < nrys_roots; i++) {~%" comp)
      (dump-s-loop-grids fout tot-bits)
      (format fout "}~%")
      (let ((assemb (to-c-code-string fout #'cell-converter-avx flat-script t)))
        (loop for s in assemb
              for gid from 0 do
              (if (equal s " 0")
                (format fout "MM_STORE(gout+ig+bgrids*(n*~a+~a), MM_SET1(0.));~%" goutinc gid)
                (format fout "MM_STORE(gout+ig+bgrids*(n*~a+~a), ~a);~%" goutinc gid s))))
      (format fout "}}} else {~%")
      (format fout "for (n = 0; n < nf; n++) {
ix = idx[0+n*3];
iy = idx[1+n*3];
...
for (i = 0; i < nrys_roots; i++) {~%" comp)
      ...
      (let ((assemb (to-c-code-string fout #'cell-converter-avx flat-script t)))
        (loop for s in assemb
              for gid from 0 do
              (if (not (equal s " 0"))
                (format fout "MM_STORE(gout+ig+bgrids*(n*~a+~a), MM_LOAD(gout+ig+bgrids*(n*~a+~a)) + ~a);~%"
                        goutinc gid goutinc gid s))))
      (format fout "}}}}~%")
      goutinc)))
